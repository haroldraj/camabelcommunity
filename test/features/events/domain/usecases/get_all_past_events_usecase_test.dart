...
void main() {
  ...
  late GetAllPastEventsUsecase usecase;

  setUp(() {
    repository = MockEventRepository();
    usecase = GetAllPastEventsUsecase(repository);
  });

  group("GetAllPastEventsUsecase", () {
    test(
      "should call repository.getAllPastEvents and return Right(List<Event>)",
      () async {
        final events = <Event>[
          Event(
            id: "id",
        ...
        when(
          () => repository.getAllPastEvents(),
        ).thenAnswer((_) async => Right(events));

        final result = await usecase(NoParams());

        expect(result, Right<Failure, List<Event>>(events));
        verify(() => repository.getAllPastEvents()).called(1);
        verifyNoMoreInteractions(repository);
      },
    );
    test(
      "should call repository.getAllPastEvents and return Left(Failure)",
      () async {
